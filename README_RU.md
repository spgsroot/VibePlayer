...
- **🌐 WebView-воспроизведение** — открытие сайта внутри приложения и использование звука страницы для вибрации
...
- **🎚️ Единые DSP-настройки** — общие smoothing, power boost и порог для Player и WebView
...
2. VibePlayer подключается к Intiface и обнаруживает поддерживаемые актуаторы
3. Звук локального плеера или WebView-страницы анализируется в реальном времени
4. Интенсивность проходит через единые DSP-настройки и отправляется через **Buttplug Protocol**
...
| `ScalarCmd` / actuator commands | Управление вибрацией, вращением, осцилляцией, позицией и другими поддерживаемыми актуаторами |
...
- **Android 10 (API 29)** или выше для вибрации от WebView-аудио
...
### Для WebView audio haptics

- Системное разрешение Android на захват аудиовыхода через MediaProjection
- Разрешение `RECORD_AUDIO`
- Foreground-уведомление во время активного анализа WebView-аудио
- Важно: пока WebView открыт, Android audio output capture может включать другой медиазвук на устройстве; VibePlayer анализирует звук локально и преобразует его только в интенсивность вибрации.
...
- Настройте общие **DSP-параметры** для всех источников:
  - **Smoothing** — стабильность реакции
  - **Порог** — отсечение тишины/шума
  - **Power Boost** — итоговый множитель интенсивности
- Установите предпочтительный **язык**

### WebView haptics

1. Откройте **Добавить видео → Открыть WebView**.
2. Введите URL сайта.
3. Разрешите Android захват аудиовыхода.
4. Запустите медиа на странице; VibePlayer проанализирует звук WebView и будет управлять выбранным Buttplug-устройством через те же DSP-настройки, что и локальный плеер.

При запуске локального плеера WebView audio capture останавливается, чтобы источники не конфликтовали за управление устройством.
...
| **WebView** | Открыть сайт и использовать звук страницы для вибрации |
...
- **📱 Runtime-разрешения** — Bluetooth и захват WebView-аудио запрашиваются только при необходимости
- **🎧 Локальный анализ звука** — аудиосэмплы анализируются на устройстве для расчёта интенсивности и не сохраняются VibePlayer
...
│   │   │   ├── domain/
│   │   │   │   ├── dsp/             # Анализ аудио, haptic mapping и runtime DSP config
│   │   │   │   └── model/           # Модели бизнес-логики
│   │   │   ├── playback/
│   │   │   │   ├── player/          # ExoPlayer wrapper и DSP audio processor
│   │   │   │   └── service/         # Foreground playback и WebView audio capture services
│   │   │   ├── ui/
│   │   │   │   ├── player/          # Экран видеоплеера
│   │   │   │   ├── webview/         # Экран WebView внутри приложения
